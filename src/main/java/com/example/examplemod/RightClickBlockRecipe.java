...
public class RightClickBlockRecipe implements Recipe<RightClickBlockInput> {
    // An in-code representation of our recipe data. This can be basically anything
    // you want.
    // Common things to have here is a processing time integer of some kind, or an
    // experience reward.
    ...
    // A list of our ingredients. Does not need to be overridden if you have no
    // ingredients
    // (the default implementation returns an empty list here). It makes sense to
    // cache larger lists in a field.
    @Override
    ...
    // Return an UNMODIFIABLE version of your result here. The result of this method
    // is mainly intended
    // for the recipe book, and commonly used by JEI and other recipe viewers as
    // well.
    @SuppressWarnings("null")
    @Override
    ...
    // Return the result of the recipe here, based on the given input. The first
    // parameter matches the generic.
    // IMPORTANT: Always call .copy() if you use an existing result! If you don't,
    // things can and will break,
    // as the result exists once per recipe, but the assembled stack is created each
    // time the recipe is crafted.
    @SuppressWarnings("null")
    @Override
    ...
    // This example outlines the most important methods. There is a number of other
    // methods to override.
    // Check the class definition of Recipe to view them all.

    @Override
    public RecipeType<?> getType() {
        return ModRecipes.RIGHT_CLICK_BLOCK_TYPE.get();
    }

    @Override
    public RecipeSerializer<?> getSerializer() {
        return ModRecipes.RIGHT_CLICK_BLOCK_SERIAL.get();
    }
